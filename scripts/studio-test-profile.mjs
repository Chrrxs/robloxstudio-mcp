...
export function parseTestProfileArguments(argv) {
  ...
  if ((mode === 'enroll' && result.command.length) ||
      (mode === 'run' && (!result.command.length || result.confirmDedicatedProfile)) ||
...
async function runProfileChild(payload) {
  ...
  if (payload.mode === 'enroll') {
    ...
    const enrolled = await runTestProfileCommand(process.execPath, [lifecycle, 'enroll-test-profile', '--source-sid', payload.sourceSid, '--confirm-dedicated-profile'], options);
    ...
    const verified = await runTestProfileCommand(process.execPath, ['tests/run-all.mjs', '--managed', '--test', 'path-resolution.mjs'], options);
    if (verified !== 0) return verified;
    return runTestProfileCommand(process.execPath, [lifecycle, 'assert-test-profile'], options);
  }
  ...
  const checked = await runTestProfileCommand(process.execPath, [lifecycle, 'assert-test-profile'], options);
  if (checked !== 0) return checked;
  const completed = await runTestProfileCommand(process.execPath, payload.command, options);
