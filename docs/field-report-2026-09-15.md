...
- **Status:** fixed in this PR — `solo_playtest action="restart"` (stop, wait for the runtime peers to drop, optional `before_start` Luau on the edit DataModel, start; `mode` preserved; `wasRunning`/`stoppedInMs`/`beforeStartMs`/`startedInMs`/`totalMs` reported). Test `tests/field-2026-09-15/02-playtest-restart.mjs` + jest `field-02-playtest-restart.test.ts`, evidence `evidence/02-playtest-restart.md`.
