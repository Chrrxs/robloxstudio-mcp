...
interface QueryHandlersModule {
  grepScripts(
    ...
    context?: { requestId: string; deadlineAt: number; isCancelled(): boolean },
...
function robloxFind(value: string, pattern: string, start = 1): [number | undefined, number | undefined] {
  ...
  if (pattern === '[\r\n]') {
    ...
    let boundary: number;
    if (newline < 0) boundary = carriageReturn;
    else if (carriageReturn < 0) boundary = newline;
    else boundary = Math.min(newline, carriageReturn);
    return boundary < 0 ? [undefined, undefined] : [boundary + 1, boundary + 1];
  }
  const index = value.indexOf(pattern, from);
...
async function loadQueryHandlers(
  ...
  const rawHandlers = loaded.default ?? loaded;
  (context as Record<string, unknown>).__HANDLERS__ = rawHandlers;
  let requestSequence = 0;
  const invokeGrep = (request: Record<string, unknown>): Record<string, unknown> => {
    requestSequence++;
    (context as Record<string, unknown>).__REQUEST_JSON__ = JSON.stringify(request);
    (context as Record<string, unknown>).__EXECUTION__ = {
      requestId: `test-request-${requestSequence}`,
      deadlineAt: Number.POSITIVE_INFINITY,
      isCancelled: () => false,
    };
    return vm.runInContext(
      '__HANDLERS__.grepScripts(JSON.parse(__REQUEST_JSON__), __EXECUTION__)',
      context,
    ) as Record<string, unknown>;
  };
  const handlers: QueryHandlersModule = { grepScripts: invokeGrep };
...
describe('Studio grep responsiveness', () => {
  ...
  test('yields large scans, prefilters misses, and rejects an overlapping grep', async () => {
    ...
    expect(harness.splitCalls()).toBe(0);
  ...
  test('rejects patterns over the Studio UTF-8 byte limit before bridging', async () => {
    const bridge = new BridgeService();
    jest.spyOn(bridge, 'resolveTarget').mockReturnValue({
      ok: true,
      mode: 'single',
